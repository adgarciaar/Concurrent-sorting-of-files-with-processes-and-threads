...
void *ProcesarArchivo(void *thread_id){
    ...
    if (tarea_id == 0){
      array1 = LeerArchivo( "archivosPrueba/log1000A", numero_lineas_archivo);
      printf("%s\n", array1[0].cadena);
      OrdenarRegistroPorBurbuja(array1, numero_lineas_archivo);
    }else{
        if (tarea_id == 1){
            array2 = LeerArchivo( "archivosPrueba/log1000B", numero_lineas_archivo);
            printf("%s\n", array2[0].cadena);
            OrdenarRegistroPorBurbuja(array2, numero_lineas_archivo);
            /*ImprimirArchivo(array2, numero_lineas_archivo, array_archivos_input[ tarea_id ], false, true);*/
        }
    }
    ...
    /*free(array_temporales[ tarea_id ]);
    array_temporales[ tarea_id ] = NULL;*/
...
/*
        for (j = 0; j < numero_lineas_archivo; j++){
          ...
          if (i == 0){
            strcpy(array_general[k].cadena, array1[j].cadena);
            array_general[k].tiempo_ejecucion = array1[j].tiempo_ejecucion;
            strcpy(array_general[k].fecha_ejecucion, array1[j].fecha_ejecucion);
            strcpy(array_general[k].hora_ejecucion, array1[j].hora_ejecucion);
              ...
              if (i == 1){
                ...
                printf("j: %d\n", j);
                printf("cadena: %s\n", array2[j].cadena);
                strcpy(array_general[k].cadena, array2[j].cadena);*/
/*
                array_general[k].tiempo_ejecucion = array2[j].tiempo_ejecucion;
                strcpy(array_general[k].fecha_ejecucion, array2[j].fecha_ejecucion);
                strcpy(array_general[k].hora_ejecucion, array2[j].hora_ejecucion);
